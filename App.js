// ============================================================
// FX CMT × HURST — MỘT APP DUY NHẤT
//
// Trang mặc định = BỘ LỌC (Screener) toàn thị trường 21 cặp ECB,
// xếp hạng bằng TÍN HIỆU CMT: cán cân bằng chứng 5 lớp × xác suất
// analog lịch sử × tỷ lệ đạt target của quy tắc breakout × khoảng
// cách tới trigger × đồng thuận đa khung. Cột Hurst đứng bên cạnh
// như thông tin tham chiếu, KHÔNG tính vào điểm.
//
// Bấm một cặp → hai tab phân tích sâu, dùng chung dữ liệu:
//   • CMT   — trình tự top-down 8 bước (vĩ mô → kế hoạch nếu-thì)
//   • Hurst — walk-forward 40 chỉ báo, chiến lược Pullback, mô phỏng
//             tài khoản theo vốn thật
//
// Dữ liệu thật, không API key: Frankfurter (ECB, 10 năm, base USD —
// mọi cặp suy từ MỘT chuỗi tỷ giá), CFTC Socrata (COT tuần), CBOE (VIX,
// có thể bị CORS). Chỗ nào không có nguồn thì UI ghi rõ, không giả lập.
//
...
const TF_BULK = ["D", "W"];
// PER-PAIR: chỉ tải thêm khi bấm mở 1 cặp cụ thể (tab CMT/Hurst/Intraday) — 3
...
/* ============================================================
   3. TOÁN NỀN (dùng chung cho cả CMT và Hurst)
   ============================================================ */
...
function backtestOHLC(
  bars,
  signals,
  { atrMult = 1.5, rr = 2, maxHold = 48 } = {}
) {
  const atrArr = atrOHLC(bars, 14);
  const trades = [];
  for (const sig of signals) {
    const entryIdx = sig.i + 1;
    if (entryIdx >= bars.length) continue;
    const atr = atrArr[sig.i];
    if (!atr) continue;
    const entry = bars[entryIdx].o;
    const sl =
      sig.dir === "long" ? entry - atr * atrMult : entry + atr * atrMult;
    const risk = Math.abs(entry - sl);
    if (!risk) continue;
    const tp = sig.dir === "long" ? entry + risk * rr : entry - risk * rr;
    let outcome = null,
      exitIdx = null;
    const lastK = Math.min(bars.length - 1, entryIdx + maxHold - 1);
    for (let k = entryIdx; k <= lastK; k++) {
      const b = bars[k];
      const hitSL = sig.dir === "long" ? b.l <= sl : b.h >= sl;
      const hitTP = sig.dir === "long" ? b.h >= tp : b.l <= tp;
      if (hitSL) {
        outcome = "sl";
        exitIdx = k;
        break;
    ...
    if (!outcome) {
      const b = bars[lastK];
      const r =
        sig.dir === "long" ? (b.c - entry) / risk : (entry - b.c) / risk;
      trades.push({
        ...sig,
        entry,
        sl,
    ...
    const r = outcome === "tp" ? rr : -1;
    trades.push({ ...sig, entry, sl, tp, r, outcome, exitIdx, atr });
  }
  return trades;
}
...
/* ============================================================
   6. HURST ENGINE — 40 chỉ báo, walk-forward, Pullback, mô phỏng vốn
   ============================================================ */
...
// Rủi ro cố định trên VỐN GỐC (không dồn lãi): mỗi lệnh rủi ro riskPct × vốn gốc.
// Nhờ vậy 3 góc nhìn (trend / range / theo CMT) cộng được với nhau và không phình lên phi thực tế.
// daily[day] = P&L của ngày, tính theo phần vốn gốc (cộng dồn tuyến tính).
...
function runPullbackWalkForward(
  ...
  return {
    ...
    winLoss: winLossStats(oosTrades, (t) => t.pips),
    sim,
    equity,
    foldDiagnostics,
    oosStart,
...
// ---- RANGE-FADE: đối chiếu khi CMT ở trong biên ----
// Chuỗi đồng thuận range (net của các chỉ báo range tin cậy) chỉ tính trong phase RANGE.
// + = quá bán (đáy biên → mua) · − = quá mua (đỉnh biên → bán). KHÔNG dùng pivot của CMT.
...
// Vào lệnh fade khi đồng thuận range vượt ngưỡng; thoát khi đã hồi về giữa (cons đổi dấu qua 0).
...
// Regime CMT theo từng phiên (giống cổng: giá đóng cửa vs biên 40 phiên). +1 trend lên · −1 trend xuống · 0 range.
...
// 3 GÓC NHÌN LỢI NHUẬN: chỉ Trend · chỉ Range(Oscillator) · Bám toàn bộ CMT (chuyển theo regime CMT).
// Tất cả walk-forward ngoài mẫu, dùng chung bộ chỉ báo tin cậy chọn trên train.
function runProfitViewsWalkForward(
  ...
  for (let f = 1; f < folds; f++) {
    const trainEnd = bnd[f],
      testStart = bnd[f],
      testEnd = bnd[f + 1];
    // Bộ chỉ báo tin cậy chọn trên train (chọn lọc theo phase là direction-agnostic)
    const best = bestCombo(
      hurstDense,
      n,
      "gated",
  ...
  // Vị thế nền: trend pullback (2 chiều) + range fade (2 chiều)
  ...
  for (let i = 0; i < n; i++) {
    trendOnly[i] = tLong[i] !== 0 ? tLong[i] : tShort[i]; // chỉ trend, tự chọn long/short theo đồng thuận
    rangeOnly[i] = rPos[i]; // chỉ oscillator, fade 2 chiều
    combined[i] =
      cmtReg[i] > 0 ? tLong[i] : cmtReg[i] < 0 ? tShort[i] : rPos[i]; // bám CMT: trend khi breakout, fade khi range
  }
  const oosStart = bnd[1];
  const mk = (pos) => {
    ...
    return {
      ...
      winLoss: winLossStats(trades, (t) => t.pips),
      equity,
    };
  };
  // % thời gian ở mỗi regime (để người dùng biết combined nghiêng về đâu)
  let cntT = 0,
    cntR = 0,
    cntTot = 0;
  for (let i = oosStart; i < n; i++) {
...
// BACKTEST "BÁM THEO THẺ CHIẾN LƯỢC": mỗi phiên, dựng lại đúng thẻ (vị trí giá + xác suất analog CAUSAL),
// nếu thẻ cho lệnh vào-ngay (fade mép / tiếp diễn) thì mô phỏng lệnh với SL/TP của chính thẻ đó.
// Không nhìn trước: analog chỉ đếm quá khứ; pivot chỉ dùng khi đã xác nhận (i.e. i ≥ pivot.i+4).
function backtestStrategyCards(closes, dates, opts, pip, digits) {
  const n = closes.length;
  const MIN_SL_PCT_CARD = 0.015; // chặn lệnh có SL quá sát giá vào — nếu không, R = lãi/lỗ ÷ khoảng SL sẽ bùng nổ ảo (vài chục/vài trăm R chỉ từ 1 lệnh) và kéo lệch toàn bộ thống kê.
  ...
  for (let i = warmup; i < n; i++) {
    ...
    if (pos) {
      ...
      if (hitTP || hitSL || i - pos.i0 >= maxHold) {
        ...
        trades.push({
          ...
          pips: (pos.side * (exit - pos.entry)) / pip,
          stoppedOut: hitSL,
          scen: pos.scen,
        });
        pos = null;
    ...
    // dựng R/S từ pivot ĐÃ xác nhận + vị trí giá; chỉ xét khi ở mép hoặc đã phá (mid-range không có lệnh ngay)
    ...
    if (!broke && posInRange < 0.7 && posInRange > 0.3) continue; // giữa biên → bỏ qua (thẻ chỉ 'chờ')
    ...
    if (
      ...
      const okSides =
        ...
        (side < 0 && st.stopY > last && st.tp1Y < last);
      const slDist = Math.abs(last - st.stopY);
      // Sàn SL: bỏ qua tín hiệu nếu stop nằm quá sát giá vào — tránh R bùng
      // nổ ảo (chia lãi/lỗ cho khoảng SL siêu nhỏ) làm lệch toàn bộ thống kê.
      if (okSides && slDist >= last * MIN_SL_PCT_CARD)
        pos = {
          i0: i,
          side,
          entry: last,
  ...
  // Thống kê + equity walk-forward (causal) chia fold để báo cáo
  const folds = Math.max(2, opts.wfFolds);
  const bnd = Array.from({ length: folds + 1 }, (_, k) =>
    Math.floor((n * k) / folds)
  );
...
function runHurstAnalysis(closes, dates, opts, direction, pipSize) {
  PIP_SIZE = pipSize || 0.0001;
  const n = closes.length;
  ...
  const { trendDefs, rangeDefs } = buildDefs(closes);
  ...
  const relCount = useAlways
    ? activeLive.allRel.length
    : liveGated.relTrend.length + liveGated.relRange.length;
  ...
  // Chiến lược RANGE-FADE (đối chiếu khi CMT ở trong biên) — fade CẢ HAI đầu biên (oscillator 2 chiều)
  ...
  // ---- SỐ LIỆU ĐỐI CHIẾU CHO BAR HIỆN TẠI (không mask theo hướng) ----
  // Hurst exponent gần nhất
  let hurstNow = null;
  ...
  // Net đồng thuận thô của 20 chỉ báo TREND gốc và 20 RANGE gốc tại bar cuối (∈ [-1,1])
  const netOf = (defs) => {
    let s = 0,
      c = 0;
    for (const d of defs)
  ...
  // 3 góc nhìn lợi nhuận (chỉ trend / chỉ range / bám toàn bộ CMT)
  ...
  // Backtest bám theo thẻ chiến lược (fade mép / tiếp diễn), causal walk-forward
  const cardBacktest = backtestStrategyCards(
    closes,
    dates,
    opts,
    pipSize || 0.0001,
    pipSize >= 1 ? 1 : pipSize >= 0.01 ? 3 : 5
  );
...
// Backtest pullback rút gọn (5 chỉ báo trend) — chỉ để hiển thị cột tham chiếu ở screener
...
function screenPair(cfg, pd, opts) {
  ...
  // mở sâu 1 cặp cụ thể ở tab CMT/Hurst/Intraday.
      ...
      <div className="grid2">
        ...
        >
          ...
          <p className="sub" style={{ marginTop: 10 }}>
            ...
            Drawdown và Sharpe của chuỗi lệnh thật cần nhật ký giao dịch — tab
            Hurst mô phỏng phần này trên quy tắc máy.
          </p>
        </Panel>
      </div>
    </>
  ...
  return (
    <>
      ...
      <Panel
        ...
        sub="Quy trình: CMT đưa ra trạng thái (breakout hoặc trong biên) và hướng; sang tab Hurst để đối chiếu — Hurst có đồng ý regime không, chỉ báo nghiêng cùng hướng không. Hurst bám indicator, không dùng biên/pivot của CMT."
      >
        <GateCascadeStrip gates={gates} />
        {gate.active ? (
          <>
            ...
            <p className="sub" style={{ margin: "0 0 10px" }}>
              CMT đang ở regime <b>Trend</b>. Sang Hurst để kiểm tra: Hurst có
              đo đúng là phase TREND (H&gt;0.5) và 20 chỉ báo Trend có đồng
              thuận {gate.dir === "long" ? "Long" : "Short"} không. Nếu khớp →
              thực thi theo công thức Pullback của Hurst.
            </p>
            <button
              className="bt"
              style={{
                borderColor: gate.dir === "long" ? CLR.bull : CLR.bear,
                color: CLR.text,
                fontWeight: 700,
              }}
              onClick={goHurst}
          ...
          <>
            ...
            <p className="sub" style={{ margin: "0 0 10px" }}>
              CMT đang ở regime <b>Range</b>. Sang Hurst để kiểm tra: Hurst có
              đo đúng là phase RANGE (H&lt;0.5) và 20 chỉ báo Range đang nghiêng
              phía nào so với lean của CMT. Nếu khớp → có thể fade biên theo
              công thức Range-Fade của Hurst; chưa vào lệnh trend cho tới khi
              CMT breakout.
            </p>
            <button
              className="bt"
...
/* ============================================================
   18. TAB HURST — MÔ PHỎNG HIỆU SUẤT
   ============================================================ */
...
function MetricBox({ label, value, sub, color }) {
  return (
    ...
    >
      ...
      <div
        className="mono"
        style={{ fontSize: 20, color: color || CLR.blue, marginTop: 2 }}
      >
        {value}
      </div>
      {sub && (
        <div style={{ fontSize: 11, color: CLR.mut, marginTop: 2 }}>{sub}</div>
      )}
    </div>
  );
}

// Tính đối chiếu CMT ↔ Hurst cho bar hiện tại
function computeConfirm(d, cmtRegime, cmtDir) {
  const H = d.hurstNow;
  const phase = d.todayPhase; // TREND | RANGE | OTHER (phase Hurst đã lọc)
  const familyNet = cmtRegime === "TREND" ? d.trendNetToday : d.rangeNetToday;
  // Regime match: phase Hurst khớp regime CMT (ưu tiên phase; H là số hỗ trợ)
  ...
  // Direction: familyNet dương = long-lean, âm = short-lean
  const netDir =
    familyNet > 0.05 ? "long" : familyNet < -0.05 ? "short" : "flat";
  const dirMatch =
    cmtDir === "side"
      ? netDir !== "flat"
        ? "lean"
  ...
  if (regimeMatch === "match" && dirMatch === "match") {
    ...
    verdict = `Hurst đồng ý regime · chỉ báo nghiêng ${
      netDir === "long" ? "Long" : "Short"
    }`;
...
const REGIME_STRAT = {
  TREND: {
    ...
    desc: "Vào khi có xu hướng + phiên giá lùi ngược; bám 20 chỉ báo Trend theo đúng hướng Long/Short của CMT.",
  },
  RANGE: {
    name: "Oscillator Fade 2 chiều",
    desc: "Fade cả hai đầu biên khi đồng thuận oscillator chạm cực trị (quá bán mua đáy, quá mua bán đỉnh); bám 20 chỉ báo Oscillator.",
  },
};
...
function HurstTab({
  ...
  if (!detail)
    return (
      <Panel
        mod="Hurst"
        title="Đang chạy walk-forward…"
        sub="Đối chiếu trạng thái CMT với Hurst cho cặp đang chọn."
      />
    );
  const d = detail;
  ...
  const color = dir === "long" ? CLR.bull : CLR.bear;

  // Chọn chiến lược khớp regime (dùng cho phần đối chiếu & giải thích công thức)
  ...
  return (
    <>
      {/* ---- ĐỐI CHIẾU CMT ↔ HURST (headline) ---- */}
      <Panel
        mod="Đối chiếu · CMT ↔ Hurst"
        title={`CMT nói ${
          cmtRegime === "TREND"
            ? `BREAKOUT ${cmtDir === "long" ? "LONG" : "SHORT"}`
            : "ĐANG TRONG BIÊN"
        } — Hurst có đồng ý không?`}
        sub="Hurst không tự đề xuất lệnh riêng; nó kiểm tra chính trạng thái CMT đưa ra: regime có khớp không (phase Hurst + hệ số H), và họ chỉ báo tương ứng nghiêng hướng nào so với CMT. Hurst bám indicator, không dùng biên/pivot của CMT."
      >
        <GateCascadeStrip gates={gates} />
        <div
          style={{
        ...
        >
          ...
          {gate && gate.active && gate.conflict && (
            <Chip cls="side">
              ⚠ Breakout còn ngược cán cân bằng chứng CMT — thận trọng
            </Chip>
          )}
          {gate && gate.active && gate.conflictW && (
            <Chip cls="side">
              ⚠ Lệnh Ngày đang ngược xu hướng khung Tuần — thận trọng
            </Chip>
          )}
        </div>
        <div className="grid3">
          <div className="scen" style={{ margin: 0 }}>
            ...
            <div className="kv" style={{ border: "none" }}>
              ...
              <span style={{ color: CLR.mut }}>
                {gate && gate.active
                  ? `phá ${gate.level.toFixed(cfg.digits)}`
                  : "giá trong biên 40 phiên"}
              </span>
            </div>
          </div>
          <div className="scen" style={{ margin: 0 }}>
            ...
            <div className="kv" style={{ border: "none" }}>
              ...
              <span>
                <Chip cls={regChip(conf.regimeMatch)}>
                  {conf.regimeMatch === "match"
                    ? "KHỚP"
                    : conf.regimeMatch === "conflict"
                    ? "NGƯỢC"
                    : "chưa rõ"}
                </Chip>
              </span>
          ...
          <div className="scen" style={{ margin: 0 }}>
            <b>3 · Chỉ báo Hurst ({isTrend ? "Trend" : "Oscillator"})</b>
            <div className="kv">
        ...
        <p className="sub" style={{ margin: "12px 0 0" }}>
          {conf.regimeMatch === "match" && conf.dirMatch === "match"
            ? `Hurst xác nhận cả regime lẫn hướng — có cơ sở thực thi theo công thức ${strat.name} dưới đây.`
            : conf.regimeMatch === "conflict"
            ? `CMT gọi ${
                cmtRegime === "TREND" ? "breakout" : "range"
              } nhưng Hurst đang ở phase ${
                conf.phase === "TREND"
                  ? "TREND"
                  : conf.phase === "RANGE"
                  ? "RANGE"
                  : "chưa rõ"
              } — hai bên đọc regime khác nhau, giảm độ tin cậy của lệnh CMT này.`
            : conf.dirMatch === "conflict"
            ? "Hurst đồng ý về regime nhưng chỉ báo đang nghiêng ngược hướng CMT — dễ là tín hiệu sớm/false, cân nhắc chờ thêm hoặc sizing nhỏ."
            : "Đối chiếu chưa dứt khoát — đọc thêm chi tiết chiến lược bên dưới trước khi hành động."}
        </p>
      </Panel>
      ...
      <Panel
        mod="Cách tính"
        title={`Công thức lợi nhuận — regime ${
          isTrend
            ? "TREND (theo trend, bám long/short CMT)"
            : "RANGE (oscillator, fade 2 đầu)"
        }`}
        sub="Hurst mở đúng họ chỉ báo theo regime CMT rồi tính lãi/lỗ từ chính tín hiệu của họ đó. Không dùng biên/pivot của CMT — chỉ dùng indicator."
      >
        <div className="grid2">
          <div
            className="scen"
            style={{
              margin: 0,
              borderColor: isTrend ? CLR.bull : CLR.line,
              opacity: isTrend ? 1 : 0.55,
            }}
          >
            <b style={{ color: isTrend ? CLR.bull : CLR.mut }}>
              Nếu CMT = TREND → mở chỉ báo TREND
            </b>
            <ol
              style={{
                margin: "8px 0 0",
            ...
            >
              <li>
                Mở 20 chỉ báo <b>trend-following</b> (MA cross, MACD, ADX-slope,
                Hull, KAMA, TEMA…), giữ lại họ tin cậy qua walk-forward.
              </li>
              <li>
                <b>Chỉ theo hướng CMT</b> ({dir === "long" ? "Long" : "Short"})
                — chỉ nhận tín hiệu {dir === "long" ? "mua" : "bán"}, bỏ tín
                hiệu ngược.
              </li>
              <li>
                Vào khi có xu hướng + phiên giá lùi ngược (pullback);{" "}
                <b>
                  SL = ATR({opts.atrPeriod}) × {opts.slMult.toFixed(1)}
                </b>
                ; thoát khi trend hết xác nhận hoặc dính SL.
              </li>
              <li>
                Lãi/lỗ mỗi lệnh = hướng × (giá ra − giá vào) ⁄ khoảng SL × (rủi
                ro% × vốn).
              </li>
            </ol>
          </div>
          <div
            ...
            style={{
              margin: 0,
              borderColor: !isTrend ? CLR.amber : CLR.line,
              opacity: !isTrend ? 1 : 0.55,
            }}
          >
            <b style={{ color: !isTrend ? CLR.amber : CLR.mut }}>
              Nếu CMT = RANGE → mở Oscillator
            </b>
            <ol
              style={{
                margin: "8px 0 0",
            ...
            >
              <li>
                Mở 20 chỉ báo <b>oscillator/mean-reversion</b> (RSI, Bollinger
                %B, Stoch, Williams %R, CCI, Z-score…), giữ họ tin cậy qua
                walk-forward.
              </li>
              <li>
                <b>Fade cả 2 đầu biên</b>: đồng thuận oscillator quá bán → mua
                đáy; quá mua → bán đỉnh (không khoá 1 hướng).
              </li>
              <li>
                SL = ATR({opts.atrPeriod}) × {opts.slMult.toFixed(1)}; thoát khi
                oscillator hồi về giữa (đồng thuận qua 0).
              </li>
              <li>Lãi/lỗ mỗi lệnh = công thức y hệt bên trái.</li>
            </ol>
          </div>
        </div>
        <p className="sub" style={{ margin: "12px 0 0" }}>
          Vốn cuối = vốn gốc luỹ kế qua từng lệnh (fixed-risk {riskPctIn.value}
          %/lệnh). Số hiển thị bên dưới lấy{" "}
          <b>đúng họ chỉ báo của regime CMT hiện tại</b> (
          {isTrend
            ? "TREND — bám " + (dir === "long" ? "Long" : "Short") + " của CMT"
            : "OSCILLATOR — fade 2 chiều"}
          ), đo bằng walk-forward ngoài mẫu.
        </p>
      </Panel>
      ...
      >
        ...
        >
          <div>
            <label className="lb">Vốn ban đầu (USD)</label>
            <input
              className="inp"
              style={{ width: 110 }}
              type="number"
              ...
              value={capital.value}
              onChange={(e) =>
                capital.set(Math.max(100, +e.target.value || 10000))
              }
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                riskPctIn.set(Math.max(0.1, Math.min(5, +e.target.value || 1)))
              }
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setOpt(
                  "atrPeriod",
                  Math.max(3, Math.min(60, +e.target.value || 14))
                )
              }
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setOpt(
                  "slMult",
                  Math.max(0.5, Math.min(6, +e.target.value || 2))
                )
              }
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setOpt(
                  "wfFolds",
                  Math.max(2, Math.min(12, +e.target.value || 5))
                )
              }
            />
          </div>
      ...
      {/* ---- Chiến lược khớp regime ---- */}
      {/* ---- LỢI NHUẬN · 3 GÓC NHÌN ---- */}
      {(() => {
        ...
        const VIEWS = {
          trend: {
            ...
            data: pv.trendOnly,
            color: CLR.bull,
            note: "Chỉ chạy chiến lược trend pullback (2 chiều, tự chọn Long/Short theo đồng thuận trend), bất kể CMT đang range hay trend.",
          },
          range: {
          ...
          combined: {
            ...
            data: pv.combined,
            color: CLR.blue,
            note: `Chỉ tính trên đúng cặp ${cfg.label}. Mỗi phiên xét regime CMT CỦA CẶP NÀY (giá đóng vs biên 40 phiên của chính ${cfg.label}): CMT breakout → trend pullback theo hướng CMT; CMT trong biên → oscillator fade. Đây là cách khớp đúng quy trình CMT ↔ Hurst cho một cặp.`,
          },
        };
        ...
        const s = cur.data.sim,
          ...
          wl = cur.data.winLoss;
        const moneyEq = cur.data.equity.map((e) => ({
          d: e.d,
          v: capital.value * (1 + e.cum / 100),
        }));
        // overlay 3 đường theo ngày
        const byDate = {};
        ...
        // So sánh THẬT giữa Combined và tổng Trend+Range — vì không dồn lãi,
        // hai số này phải gần nhau; lệch nhiều là dấu hiệu có outlier/dồn lãi
        // ở đâu đó trong cách tính, không phải một câu chữ đóng đinh sẵn.
        const sumTR = endCum("trend") + endCum("range");
        const combinedV = endCum("combined");
        const gapAbs = Math.abs(combinedV - sumTR);
        const gapDenom = Math.max(Math.abs(sumTR), 10); // sàn 10 điểm % để tránh chia cho số quá nhỏ
        const gapRatio = gapAbs / gapDenom;
        const cumCheckOff = gapRatio > 0.5; // lệch >50% so với tổng → đáng ngờ
        return (
          <Panel
            mod="Hurst · Lợi nhuận (3 góc nhìn)"
            title={`${cfg.label} — chỉ Trend · chỉ Range · theo regime CMT`}
            sub={`Tất cả tính riêng cho cặp ${
              cfg.label
            } (không phải toàn thị trường). Rủi ro CỐ ĐỊNH ${
              riskPctIn.value
            }%/lệnh trên vốn gốc ${fmtMoney(
              capital.value
            )} (không dồn lãi) — nhờ vậy 3 đường cộng được và "theo CMT" ≈ trend + range. SL = ATR(${
              opts.atrPeriod
            })×${opts.slMult.toFixed(1)}, walk-forward ngoài mẫu từ ${
              pv.oosFromDate
            }. Riêng cặp ${
              cfg.label
            } trong kỳ OOS ở trend ${pv.pctTrendTime.toFixed(
              0
          ...
          >
              ...
              {["trend", "range", "combined"].map((k) => (
                ...
                >
                  ...
                  <span
                    className="num"
                    style={{
                      color: endCum(k) >= 0 ? CLR.bull : CLR.bear,
                      marginLeft: 4,
                    }}
                  >
                    {endCum(k) >= 0 ? "+" : ""}
              ...
              <ResponsiveContainer>
                ...
                >
                  ...
                  <Line
                    ...
                    stroke={CLR.bull}
                    dot={false}
                    strokeWidth={profitView === "trend" ? 2.2 : 1}
                    strokeOpacity={profitView === "trend" ? 1 : 0.45}
                    isAnimationActive={false}
                    connectNulls
                  />
                  <Line
                    ...
                    stroke={CLR.amber}
                    dot={false}
                    strokeWidth={profitView === "range" ? 2.2 : 1}
                    strokeOpacity={profitView === "range" ? 1 : 0.45}
                    isAnimationActive={false}
                    connectNulls
                  />
                  <Line
                    ...
                    stroke={CLR.blue}
                    dot={false}
                    strokeWidth={profitView === "combined" ? 2.2 : 1}
                    strokeOpacity={profitView === "combined" ? 1 : 0.45}
                    isAnimationActive={false}
                    connectNulls
                  />
                </LineChart>
            ...
            >
              <Chip cls="mut">
                Đang xem:{" "}
                <b style={{ color: cur.color, marginLeft: 4 }}>{cur.label}</b>
              </Chip>
            </div>
            ...
            >
              ...
              />
              <MetricBox
                label="Sharpe (theo lệnh)"
                value={isFinite(ts.sharpe) ? ts.sharpe.toFixed(2) : "—"}
                sub={`${ts.count} lệnh · giữ TB ${
                  ts.avgHoldDays ? ts.avgHoldDays.toFixed(1) : "—"
                } phiên`}
              />
              <MetricBox
                label="Tỷ lệ thắng"
                value={isFinite(ts.hitRate) ? ts.hitRate.toFixed(0) + "%" : "—"}
              />
              <MetricBox
                label="Max Drawdown"
                value={`${s.maxDDPct.toFixed(1)}%`}
                color={CLR.bear}
                sub={fmtMoney((capital.value * Math.abs(s.maxDDPct)) / 100)}
              />
              <MetricBox
                label="% lệnh dừng SL"
                value={
                  isFinite(s.stoppedOutPct)
                    ? s.stoppedOutPct.toFixed(0) + "%"
                    : "—"
                }
              />
            </div>
            ...
            <div style={{ width: "100%", height: 160 }}>
              <ResponsiveContainer>
                ...
                >
                  ...
                  <Tooltip
                    contentStyle={TT}
                    formatter={(v) => [fmtMoney(v), "Giá trị tài khoản (OOS)"]}
                    labelStyle={{ color: CLR.blue }}
                  />
                </LineChart>
              </ResponsiveContainer>
            </div>
            ...
            <div style={{ overflowX: "auto", marginTop: 12 }}>
              <table className="tbl" style={{ minWidth: 620 }}>
                ...
                <tbody>
                  <tr>
                    <td style={{ color: CLR.bull, fontWeight: 800 }}>
                      Lệnh THẮNG (pip)
                    </td>
                    <td className="num">{wl.win ? wl.win.n : 0}</td>
                    <td className="num" style={{ color: CLR.bull }}>
                      {wl.win ? pipTxt(wl.win.min) : "—"}
                    </td>
                    <td className="num" style={{ color: CLR.bull }}>
                      {wl.win ? pipTxt(wl.win.median) : "—"}
                    </td>
                    <td
                      className="num"
                      style={{ color: CLR.bull, fontWeight: 700 }}
                    >
                      {wl.win ? pipTxt(wl.win.mean) : "—"}
                    </td>
                    <td className="num" style={{ color: CLR.bull }}>
                      {wl.win ? pipTxt(wl.win.max) : "—"}
                    </td>
                  </tr>
                  <tr>
                    <td style={{ color: CLR.bear, fontWeight: 800 }}>
                      Lệnh THUA (pip)
                    </td>
                    <td className="num">{wl.loss ? wl.loss.n : 0}</td>
                    <td className="num" style={{ color: CLR.bear }}>
                      {wl.loss ? pipTxt(wl.loss.min) : "—"}
                    </td>
                    <td className="num" style={{ color: CLR.bear }}>
                      {wl.loss ? pipTxt(wl.loss.median) : "—"}
                    </td>
                    <td
                      className="num"
                      style={{ color: CLR.bear, fontWeight: 700 }}
                    >
                      {wl.loss ? pipTxt(wl.loss.mean) : "—"}
                    </td>
                    <td className="num" style={{ color: CLR.bear }}>
                      {wl.loss ? pipTxt(wl.loss.max) : "—"}
                    </td>
                  </tr>
                </tbody>
              </table>
            ...
            <div
              ...
              style={{
                margin: "12px 0 0",
                borderColor: cumCheckOff
                  ? "rgba(255,90,90,.5)"
                  : "rgba(110,168,255,.4)",
              }}
            >
              <b>
                Kiểm tra cộng dồn (vì không dồn lãi nên phải xấp xỉ bằng nhau)
              </b>
              <div className="kv" style={{ border: "none", paddingTop: 6 }}>
                <span>
                  Chỉ Trend ({endCum("trend") >= 0 ? "+" : ""}
                  {endCum("trend").toFixed(0)}%) + Chỉ Range (
                  {endCum("range") >= 0 ? "+" : ""}
                  {endCum("range").toFixed(0)}%)
                </span>
                <span className="num">
                  ≈ {(endCum("trend") + endCum("range")).toFixed(0)}%
                </span>
              </div>
              <div className="kv" style={{ border: "none" }}>
                <span>Theo regime CMT (thực tế)</span>
                <span
                  className="num"
                  style={{ color: cumCheckOff ? CLR.bear : CLR.blue }}
                >
                  {endCum("combined") >= 0 ? "+" : ""}
                  {endCum("combined").toFixed(0)}%
                </span>
              </div>
              <p
                className="sub"
                style={{
                  margin: "6px 0 0",
                  fontSize: 11,
                  color: cumCheckOff ? CLR.bear : CLR.dim,
                  fontWeight: cumCheckOff ? 700 : 400,
                }}
              >
                {cumCheckOff
                  ? `⚠ LỆCH ${(gapRatio * 100).toFixed(
                      0
                    )}% so với tổng Trend+Range — vượt ngưỡng cho phép (chênh nhỏ do vài lệnh bị SL cắt lúc chuyển regime). Có khả năng đang dồn lãi hoặc có lệnh outlier ở đâu đó trong cách tính "theo regime CMT" — đừng vội tin số Combined ở trên, nên kiểm tra lại trước khi dùng.`
                  : `Hai số này gần nhau (lệch ${(gapRatio * 100).toFixed(
                      0
                    )}%, trong ngưỡng cho phép do vài lệnh bị SL cắt ngay lúc chuyển regime) — không có dấu hiệu dồn lãi.`}
              </p>
            </div>
            <p className="sub" style={{ marginTop: 10, fontSize: 11 }}>
              ⚠ Walk-forward ngoài mẫu, khớp tại giá đóng cửa (
              {cfg.crypto ? "Binance daily" : "ECB fixing"}), chưa trừ
              phí/spread/swap. Rủi ro cố định trên vốn gốc (không dồn lãi) nên
              đây là % cộng dồn tuyến tính, không phải tăng trưởng kép của tài
              khoản thật. "Theo regime CMT" là góc nhìn khớp đúng quy trình cho
              riêng {cfg.label}: trend khi cặp này breakout, oscillator khi cặp
              này trong biên.
            </p>
          </Panel>
        );
      ...
      {/* ---- BACKTEST BÁM THEO THẺ CHIẾN LƯỢC ---- */}
      {(() => {
        ...
        const scenRows = Object.entries(cb.byScen).sort(
          (a, b) => b[1].n - a[1].n
        );
        // So sánh THẬT: tổng lợi nhuận headline (đã quy theo % vốn gốc) phải
        // ≈ tổng R cộng dồn từ các loại thẻ × rủi ro%/lệnh — vì rủi ro cố
        // định và chỉ 1 lệnh mở tại một thời điểm (không chồng lệnh) nên hai
        // số này PHẢI khớp nhau, xấp xỉ; lệch nhiều là dấu hiệu vẫn còn lỗi
        // trong cách tính (outlier R, hoặc credit sai ngày) chứ không phải
        // "may mắn chọn đúng lúc".
        const scenRSum = scenRows.reduce((s, [, v]) => s + v.R, 0);
        const impliedPct = scenRSum * riskPctIn.value;
        const cbGapDenom = Math.max(Math.abs(impliedPct), 10);
        const cbGapRatio = s.blown
          ? 0
          : Math.abs(s.totalReturnPct - impliedPct) / cbGapDenom;
        const cbCheckOff = !s.blown && cbGapRatio > 0.5;
        return (
          <Panel
            mod="Hurst · Backtest thẻ chiến lược"
            title={`${cfg.label} — nếu bám theo thẻ chiến lược thì lời/lỗ ra sao?`}
            sub={`Mỗi phiên dựng lại đúng thẻ (vị trí giá + xác suất A/B/C tính CAUSAL — chỉ dùng quá khứ), nếu thẻ cho lệnh vào-ngay (fade mép / tiếp diễn) thì vào lệnh với SL/TP của chính thẻ. Không nhìn trước. Vốn ${fmtMoney(
              capital.value
            )}, rủi ro ${
              riskPctIn.value
            }%/lệnh cố định trên vốn gốc, giữ tối đa ${
              opts.cardMaxHold || 30
            } phiên. OOS từ ${cb.oosFromDate}.`}
          >
            {cb.trades.length < 5 ? (
              <Warn>
                Chỉ có {cb.trades.length} lệnh trong kỳ OOS — quá ít để kết
                luận. Thẻ chiến lược của {cfg.label} hiếm khi cho tín hiệu
                vào-ngay (giá ít khi ở mép với cấu hình xác suất phù hợp).
              </Warn>
            ) : (
              <>
                <div
                ...
                >
                  <MetricBox
                    ...
                    color={s.totalReturnPct >= 0 ? CLR.bull : CLR.bear}
                    sub={
                      s.blown ? "" : fmtMoney(capital.value * s.finalMultiple)
                    }
                  />
                  <MetricBox
                    label="Số lệnh"
                  ...
                  <MetricBox
                    label="Tỷ lệ thắng"
                    value={
                      isFinite(ts.hitRate) ? ts.hitRate.toFixed(0) + "%" : "—"
                    }
                  />
                  <MetricBox
                    label="Sharpe (theo lệnh)"
                ...
                <div style={{ width: "100%", height: 150 }}>
                  <ResponsiveContainer>
                    ...
                    >
                      ...
                      <Tooltip
                        contentStyle={TT}
                        formatter={(v) => [fmtMoney(v), "Giá trị tài khoản"]}
                        labelStyle={{ color: CLR.blue }}
                      />
                    </LineChart>
                  </ResponsiveContainer>
                ...
                <div className="grid2" style={{ marginTop: 14 }}>
                        ...
                        {scenRows.map(([k, v]) => (
                          <tr key={k}>
                            ...
                            <td
                              className="num"
                              style={{
                                color: v.win / v.n >= 0.5 ? CLR.bull : CLR.bear,
                              }}
                            >
                            ...
                            <td
                              className="num"
                              style={{ color: v.R >= 0 ? CLR.bull : CLR.bear }}
                            >
                              {v.R >= 0 ? "+" : ""}
                              {v.R.toFixed(1)}R
                            </td>
                  ...
                  <div>
                    <div className="sub" style={{ marginBottom: 4 }}>
                      Theo đoạn thời gian (walk-forward)
                    </div>
                    <table className="tbl">
                      <thead>
                        <tr>
                        ...
                        {cb.foldStats.map((f) => (
                          <tr key={f.fold}>
                            ...
                            <td
                              className="num"
                              style={{
                                color: f.totR >= 0 ? CLR.bull : CLR.bear,
                              }}
                            >
                              {f.totR >= 0 ? "+" : ""}
                              {f.totR}R
                ...
                <p className="sub" style={{ marginTop: 12, fontSize: 11 }}>
                  Đọc kỹ: nếu tổng R dương ở HẦU HẾT các đoạn (cột "Tổng R" các
                  fold) thì thẻ có tính bền; nếu chỉ 1–2 đoạn kéo cả kết quả thì
                  là may rủi. Vẫn khớp tại giá đóng cửa (
                  {cfg.crypto ? "Binance" : "ECB"}), chưa trừ phí — trừ phí vào
                  thường ăn mòn đáng kể loại lệnh fade ngắn này.
                </p>
                <div
                  className="scen"
                  style={{
                    margin: "12px 0 0",
                    borderColor: cbCheckOff
                      ? "rgba(255,90,90,.5)"
                      : "rgba(110,168,255,.4)",
                  }}
                >
                  <b>Kiểm tra cộng dồn (tổng R các loại thẻ × rủi ro%/lệnh)</b>
                  <div className="kv" style={{ border: "none", paddingTop: 6 }}>
                    <span>
                      Tổng R ({scenRSum.toFixed(1)}R) × {riskPctIn.value}%/lệnh
                    </span>
                    <span className="num">
                      ≈ {impliedPct >= 0 ? "+" : ""}
                  ...
                  <div className="kv" style={{ border: "none" }}>
                    <span>Tổng lợi nhuận OOS (thực tế)</span>
                    <span
                      className="num"
                      style={{ color: cbCheckOff ? CLR.bear : CLR.blue }}
                    >
                      {s.blown
                        ? "⚠ Cháy TK"
                        : `${
                            s.totalReturnPct >= 0 ? "+" : ""
                          }${s.totalReturnPct.toFixed(0)}%`}
                    </span>
                  </div>
                  <p
                    className="sub"
                    style={{
                      margin: "6px 0 0",
                      fontSize: 11,
                      color: cbCheckOff ? CLR.bear : CLR.dim,
                      fontWeight: cbCheckOff ? 700 : 400,
                    }}
                  >
                    {cbCheckOff
                      ? `⚠ LỆCH ${(cbGapRatio * 100).toFixed(
                          0
                        )}% so với tổng R quy đổi — vượt ngưỡng cho phép. Nhiều khả năng vẫn còn lệnh outlier hoặc lỗi tính ngày vào lệnh; đừng vội tin số Tổng lợi nhuận OOS ở trên.`
                      : `Hai số này gần nhau (lệch ${(cbGapRatio * 100).toFixed(
                          0
                        )}%, trong ngưỡng cho phép) — không có dấu hiệu outlier/lỗi cộng dồn.`}
                  </p>
                </div>
              </>
            )}
      ...
      <Panel
        ...
        title={`${cfg.label} — họ chỉ báo ${
          isTrend ? "TREND" : "OSCILLATOR"
        } (họ đang đối chiếu)`}
        sub={`Dữ liệu ${d.n.toLocaleString("vi-VN")} phiên, ${d.dates[0]} → ${
          d.lastDate
        }. Đang tô đậm họ ${
      ...
      >
        <div
          style={{
            ...
            height: Math.max(
              420,
              (isTrend ? d.trendResults : d.rangeResults).length * 19
            ),
          }}
        >
          <ResponsiveContainer>
            <BarChart
              data={isTrend ? d.trendResults : d.rangeResults}
              layout="vertical"
              margin={{ top: 6, right: 20, bottom: 0, left: 10 }}
            >
              <CartesianGrid
                stroke={CLR.line}
                strokeDasharray="2 4"
              ...
              <YAxis
                type="category"
                dataKey="label"
                tick={{ fill: CLR.text, fontSize: 10 }}
                width={170}
              />
              ...
              <Tooltip
                contentStyle={TT}
                formatter={(v, n2, pl) => [
                  `Sharpe ${v.toFixed(2)} (số lệnh=${pl.payload.activeN})`,
                  "",
                ]}
              />
      ...
      <Panel
        mod="Hurst · Walk-forward"
        title="Kiểm định theo đoạn (chung cho 3 góc nhìn)"
        sub="Mỗi đoạn: tối ưu bộ chỉ báo + ngưỡng Hurst trên phần train, đo trên phần test ngay sau đó — không nhìn trước. Cùng bộ chỉ báo tin cậy này nuôi cả 3 chiến lược lợi nhuận ở trên."
      >
        <div style={{ overflowX: "auto" }}>
          <table className="tbl" style={{ minWidth: 560 }}>
            ...
            <tbody>
              {d.profitViews.foldDiagnostics.map((f) => (
                <tr key={f.fold}>
                  <td>#{f.fold}</td>
                  <td className="num">{f.testFromDate}</td>
                  <td className="num">{f.testToDate}</td>
                  <td className="num">±{f.buffer.toFixed(2)}</td>
                  <td className="num">{f.stableWin}</td>
                  <td className="num">{f.relTrendCount}</td>
                  <td className="num">{f.relRangeCount}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <Warn>
        ...
        độ tin, ngược thì cảnh báo. Số backtest đo "quy tắc như đã cài đặt" trên
        giá đóng cửa ECB, chưa trừ phí/spread/swap. Quyết định vào lệnh vẫn dựa
        trên phân tích CMT ở tab bên cạnh. Không phải khuyến nghị đầu tư.
      </Warn>
    </>
  );
}
...
function buildCMTModel(barsByTF, dxySeries, vixSeries, cot, seas, cfg) {
  // Ngày là khung "trục" cho toàn bộ engine Close-based bên dưới (RSI/MACD/Hurst/
  ...
  // Chỉ khi cổng này BẬT thì tab Hurst mới đưa công thức thực thi; hướng lấy từ đây.
...
export default function App() {
  ...
  const [view, setView] = useState("screener"); // screener | cmt | hurst | intraday
  ...
  // Cổng vào lệnh CMT quyết định hướng Hurst khi để "auto":
  // ưu tiên chiều CMT đang trong lệnh; nếu chưa vào lệnh thì lấy chiều bias của screener.
  ...
  const resolvedDir = useMemo(() => {
    ...
    const row = screener && screener.find((r) => r.key === pairKey);
    return row ? (row.bias === "down" ? "short" : "long") : "long";
  }, [dirChoice, gate, screener, pairKey]);

  const hurstOpts = useMemo(
    () => ({ ...opts, riskPct: Math.max(0.1, riskPctIn) / 100 }),
    [opts, riskPctIn]
  );
  const hurstCache = useRef(new Map());
  const detail = useMemo(() => {
    if (!cfg || view !== "hurst") return null;
    const pd = pairData(cfg);
    if (!pd) return null;
    const key = `${pairKey}|${resolvedDir}|${JSON.stringify(hurstOpts)}`;
    if (hurstCache.current.has(key)) return hurstCache.current.get(key);
    const res = runHurstAnalysis(
      pd.D.map((b) => b.c),
      pd.D.map((b) => b.d.slice(0, 10)),
      hurstOpts,
      resolvedDir,
      cfg.pip
    );
    ...
    return res;
  }, [ohlcStore, cfg, pairData, view, pairKey, resolvedDir, hurstOpts, gate]);
    ...
    <div className="fxapp">
      ...
      <header className="topbar">
        <div className="brand">
          FX·CMT × HURST
          <small>
            Bộ lọc tín hiệu CMT · phân tích top-down · mô phỏng hiệu suất
          </small>
        </div>
        {view !== "screener" && (
          <>
      ...
      <div className="tabs">
        ...
        <button
          className={`tab ${view === "hurst" ? "on" : ""}`}
          onClick={() => setView("hurst")}
        >
          {gate && gate.active ? (
            <span style={{ color: gate.dir === "long" ? CLR.bull : CLR.bear }}>
              ●
            </span>
          ) : (
            "○"
          )}{" "}
          Hurst — đối chiếu CMT ({cfg.label})
        </button>
        <button
